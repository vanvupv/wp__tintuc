...
function wpcore_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'wpcore' ),
		)
	);
...
function wpcore_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'wpcore_content_width', 640 );
}
...
function wpcore_scripts() {
	wp_enqueue_style( 'wpcore-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'wpcore-style', 'rtl', 'replace' );

	wp_enqueue_script( 'wpcore-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
